...
genai.configure(api_key=os.getenv('GOOGLE_API_KEY', settings.GOOGLE_API_KEY))
model = genai.GenerativeModel('gemini-2.5-flash')
prompt = CLASSIFICATION_PROMPT.format(business_type='medicine', business_description='general medicine', ocr_text=text)
res = model.generate_content(prompt)

with open('test_25_raw.txt', 'w', encoding='utf-8') as f:
    f.write(res.text)
